...
# 创建状态、动作空间
S = [i for i in range(25)]# 状态空间
A=["n","e","s","w"] # 动作空间 (n:北  e:东  s:南  w:西)
ds_actions={"n":-5,"e":1,"s":5,"w":-1}
...
def dynamics(s,a):# 环境动力学
    ...
    '''
    s_prime = s
    if (s % 5 == 0 and a == "w") or (s < 5 and a == "n") or ((s + 1) % 5 == 0 and a == "e") or (s > 19 and a == "s") or s in[4, 20]:
        pass
    else:
        ds = ds_actions[a]
        s_prime = s + ds
    reward = 0 if s in [4, 20] else -1
    is_end = True if s in [4, 20] else False
...
#测试随机策略
V = [0 for _ in range(25)]  # 状态价值
V_pi = policy_evaluate(MDP, V, uniform_random_pi, 10)
print("策略评估: 使用随机策略")
display_V(V_pi)
#测试贪婪策略
V = [0 for _ in range(25)]  # 状态价值
V_pi = policy_evaluate(MDP, V, greedy_pi, 4)
print("策略评估: 使用贪婪策略")
display_V(V_pi)
...
V = [0 for _ in range(25)]# 状态价值
V_pi = policy_evaluate(MDP, V, uniform_random_pi, 800)
display_V(V_pi)
